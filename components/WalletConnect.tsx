            ...
            <div className="mx-2 flex flex-row items-center justify-between gap-2 py-0.5 pl-3 pr-2">
              ...
              {walletState.adaHandle?.total_handles && (
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    document.dispatchEvent(new CustomEvent('showHandlesDialog'))
                  }}
                  className="whitespace-nowrap text-muted-foreground transition-colors hover:text-foreground"
                >
                  {walletState.adaHandle.total_handles.toLocaleString()}{' '}
                  <span className="text-green-500 dark:text-green-400">$handles</span>
                </button>
              )}
              ...
              {Array.from(
                new Set(
                  walletState.supportedWallets.map(
                    (wallet: string) => window.cardano?.[wallet]?.icon,
                  ),
                ),
              )
                ...
                .map((icon, index) => {
                  ...
                  const walletName = wallet
                    ? wallet.charAt(0).toUpperCase() + wallet.slice(1).toLowerCase()
                    : ''
